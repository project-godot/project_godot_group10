...
func chase_state(_delta):
	if player_node and is_instance_valid(player_node):
		...
		if dist > 350:  # Aumentado um pouco para não perder o player facilmente
			current_state = State.PATROL
			player_node = null
			return
		...
		if dist < ATTACK_RANGE and not is_attacking and attack_cooldown_timer <= 0:
			...
			# A animação tem ~1.2 segundos (12 frames / speed 10.0)
			var attack_duration = 1.2  # Duração completa da animação de ataque
...
func check_for_player_manually():
	...
	if players.size() > 0:
		...
		# Aumentar o range de detecção manual para 300 pixels
		if distance < 300:
